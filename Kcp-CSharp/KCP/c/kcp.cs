...
namespace kcp
{
    ...
    public static unsafe partial class KCP
    {
        ...
        // output segment
        public static void ikcp_output(IKCPCB* kcp, byte* destination, int size, KcpCallback output)
        {
            assert(kcp != null);
            assert(output != null);
        ...
        public static IKCPCB* ikcp_create(uint conv, int reserved)
        {
            ...
            kcp->buffer = (byte*)ikcp_malloc((nuint)((reserved + kcp->mtu + IKCP_OVERHEAD) * 3));
            if (kcp->buffer == null)
            {
                ikcp_free(kcp);
                return null;
        ...
        public static void ikcp_update(IKCPCB* kcp, uint current, int reserved, KcpCallback output)
        {
            int slap;
            ...
            if (slap >= 0)
            {
                ...
                ikcp_flush(kcp, reserved, output);
            }
        }
        ...
        public static int ikcp_setmtu(IKCPCB* kcp, int mtu, int reserved)
        {
            ...
            if (mtu < 50 || mtu < (int)IKCP_OVERHEAD)
                return -1;
            buffer = (byte*)ikcp_malloc((nuint)((reserved + mtu + IKCP_OVERHEAD) * 3));
            if (buffer == null)
                return -2;
            kcp->mtu = (uint)mtu;
            kcp->mss = kcp->mtu - IKCP_OVERHEAD;
