    ...
    internal static unsafe class IKCP
    {
...
#else
            (void*)Marshal.AllocHGlobal((int)size);
#endif
        ...
        public static int ikcp_send(IKCPCB* kcp, byte* buffer, int len)
        {
            ...
            if (kcp->stream != 0)
            {
                ...
                if (len <= 0)
                    return sent;
                var count = ikcp_frg_count(kcp, len);
                int i;
                for (i = 0; i < count; ++i)
                {
        ...
        private static int ikcp_frg_count(IKCPCB* kcp, int len)
        {
            int count;
            if (len <= (int)kcp->mss)
                count = 1;
        ...
        {
            interval = _iclamp_(interval, INTERVAL_MIN, INTERVAL_MAX);
        ...
        {
            kcp->snd_wnd = (uint)_iclamp_(sndwnd, WND_SND, 2147483647);
            kcp->rcv_wnd = (uint)_iclamp_(rcvwnd, WND_RCV, 2147483647);
        }
